          ...
          <div className="space-y-2 mb-4">
            ...
            {booking.reservationType?.toLowerCase().includes('hotel') && (booking.rooms?.length || 0) > 0 ? (
              <>
                ...
                <div className="flex items-center gap-2 text-gray-700">
                  ...
                  <span className="text-sm">
                    {formatDate(String(booking.rooms?.[0].checkInDate))} – {formatDate(String(booking.rooms?.[0].checkOutDate))}
                  </span>
                </div>
                <div className="flex items-center gap-2 text-gray-700">
                  ...
                  <span className="text-sm">
                    ...
                    {(booking.rooms?.reduce((sum, r) => sum + (r.guests || 0), 0) || 0) > 1 ? 's' : ''},{' '}
                    {booking.rooms?.reduce((sum, r) => sum + (r.quantity || 1), 0)} Room
                    {(booking.rooms?.reduce((sum, r) => sum + (r.quantity || 1), 0) || 0) > 1 ? 's' : ''}
                  </span>
                </div>
              </>
            ) : (
              <>
                <div className="flex items-center gap-2 text-gray-700">
                  <Calendar className="w-4 h-4 flex-shrink-0" />
                  <span className="text-sm">{formatDate(String(booking.date))}</span>
                </div>
                <div className="flex items-center gap-2 text-gray-700">
                ...
                <div className="flex items-center gap-2 text-gray-700">
                  <Users className="w-4 h-4 flex-shrink-0" />
                  <span className="text-sm">{booking.guests} Guest{(booking.guests || 0) > 1 ? 's' : ''}</span>
                </div>
              </>
            )}
