...
interface MenuReservationProps {
  menuFiltered: MenuListItem[];
  setMenuItems: Dispatch<SetStateAction<MenuListItem[]>>;
  menuItems: MenuListItem[];
}

const MenuReservation = ({ menuFiltered, setMenuItems, menuItems }: MenuReservationProps) => {
      ...
      <div className="space-y-4 mb-6">
        {menuFiltered.length > 0
          ? menuFiltered.slice(0, index).map((item, i) => (
              <Card key={i} className={`overflow-hidden ${item.selected ? 'border-teal-500' : ''}`}>
                      ...
                      <div className="mt-4 flex flex-col sm:flex-row sm:items-center gap-3">
                        ...
                        <div className="flex-1">
                          <Input
                            ...
                            onChange={(e) => handleSpecialRequestChange(item._id, e.target.value)}
