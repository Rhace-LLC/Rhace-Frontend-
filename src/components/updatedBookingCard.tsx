...
function BookingCard({ booking, onEdit, onCancel }: BookingCardProps) {
  ...
  const payment = booking.paymentRefs && booking.paymentRefs.length > 0 ? booking.paymentRefs[0] : null;
  ...
  const checkInContext = (() => {
    ...
    if (daysUntilCheckIn < 0) return `Checked in ${formatDate(String(checkInDate))}`;
  ...
  const actionButtonText: string = 'View Booking';
    ...
    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden relative">
      ...
      <div className="p-5 space-y-5">
        ...
        <div className="grid md:grid-cols-2 gap-3">
          {booking.reservationType?.toLowerCase().includes('hotel') && (booking.rooms?.length || 0) > 0 ? (
            <>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="font-medium">
                    {formatDate(booking.rooms?.[0]?.checkInDate as Date)} – {formatDate(booking.rooms?.[0]?.checkOutDate as Date)}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="font-medium">
                    ...
                    {(booking.rooms?.reduce((s, r) => s + (r.guests || 0), 0) || 0) > 1 ? 's' : ''},{' '}
                    {booking.rooms?.reduce((s, r) => s + (r.quantity || 1), 0)} Room
                    {(booking.rooms?.reduce((s, r) => s + (r.quantity || 1), 0) || 0) > 1 ? 's' : ''}
                  </p>
                </div>
              </div>
            </>
            ...
            <>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="font-medium">{formatDate(booking.date as Date)}</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="font-medium">{booking.guests} Guest{booking.guests || 0 > 1 ? 's' : ''}</p>
                </div>
              </div>
            </>
          )}

          {booking.reservationType?.toLowerCase().includes('club') && (booking.tables?.length || 0) > 0 && (
            <div className="flex items-center gap-3">
          ...
          {booking.reservationType?.toLowerCase().includes('restaurant') && (booking.menus?.length || 0) > 0 && (
            <div className="flex items-center gap-3">
