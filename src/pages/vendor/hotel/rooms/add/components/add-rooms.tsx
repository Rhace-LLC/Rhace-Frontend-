...
interface AddRoomTypeProps {
  onSubmit: (data: { roomTypes: string[] }) => void;
  ...
  handleInputChange: (roomId: string, field: string, value: unknown) => void;
}
...
interface RoomTypeFormProps {
  ...
  handleAddCustomAmenity: (roomId: string) => void;
  handleDrag: (e: React.DragEvent) => void;
  handleDrop: (e: React.DragEvent, roomId: string) => void;
  fileInputRefs: React.MutableRefObject<Record<string, HTMLInputElement | null>>;
  handleFileSelect: (e: React.ChangeEvent<HTMLInputElement>, roomId: string) => void;
}
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 p-6">
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          <Select
            value={(room.category ?? 1).toString()}
            onValueChange={(value) => handleInputChange(room.id, 'category', value)}
          >
            ...
            <SelectContent>
              {['Standard', 'Deluxe', 'Executive', 'Suite', 'Presidential Suite'].map((text) => (
                <SelectItem key={text} value={text.toString()}>
                  {text}
                </SelectItem>
              ))}
            </SelectContent>
        ...
        <div className="space-y-2">
          ...
          <Select
            value={(room.discount ?? 1).toString()}
            onValueChange={(value) => handleInputChange(room.id, 'discount', Number(value))}
          >
            <SelectTrigger className="w-full">
              <SelectValue />
            </SelectTrigger>
        ...
        <div className="space-y-2">
          <Label
            htmlFor={`adultsCapacity-${room.id}`}
            className="text-sm font-medium text-gray-700"
          >
            Adults Capacity
          </Label>
          <Select
            value={(room.adultsCapacity ?? 1).toString()}
            onValueChange={(value) => handleInputChange(room.id, 'adultsCapacity', Number(value))}
          >
            <SelectTrigger className="w-full">
              <SelectValue />
            </SelectTrigger>
        ...
        <div className="space-y-2">
          <Label
            htmlFor={`childrenCapacity-${room.id}`}
            className="text-sm font-medium text-gray-700"
          >
            Children Capacity
          </Label>
          <Select
            value={(room.childrenCapacity ?? 0).toString()}
            onValueChange={(value) => handleInputChange(room.id, 'childrenCapacity', Number(value))}
          >
            <SelectTrigger className="w-full">
              <SelectValue />
            </SelectTrigger>
        ...
        <div className="space-y-2">
          ...
          <Select
            value={(room.totalAvailableRooms ?? 1).toString()}
            onValueChange={(value) =>
              handleInputChange(room.id, 'totalAvailableRooms', Number(value))
            }
          >
            <SelectTrigger className="w-full">
              <SelectValue />
    ...
    useEffect(() => {
      ...
      return () => {
        ...
        Object.values(refs).forEach((input) => {
          const el = input as HTMLInputElement & { _objectUrls?: string[] };
          if (el && el._objectUrls) {
            el._objectUrls.forEach((url) => URL.revokeObjectURL(url));
          }
        });
      };
    }, []);
    ...
    const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>, roomId: string) => {
      if (e.target.files) {
        ...
        if (room) {
          ...
          const el = fileInputRefs.current && fileInputRefs.current[roomId];
          if (el) {
            const tracked = el as HTMLInputElement & { _objectUrls?: string[] };
            tracked._objectUrls = (tracked._objectUrls || []).concat(
              files.filter((f) => f instanceof File).map((f) => URL.createObjectURL(f))
            );
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          >
            {roomTypes.map((room) => (
              ...
              >
                <AccordionTrigger
                  className={`px-6 py-0 hover:no-underline ${openAccordion === `room-${room.id}` ? 'hidden' : 'flex items-center justify-between'}`}
                >
