...
const tableCategoryIcons: Record<string, React.ComponentType<{ className?: string }>> = {
  VIP: Crown,
  VVIP: Sparkles,
  Regular: Users,
  'Super Regular': Users,
...
export default function ReservationDetails({ id }: { id?: string }) {
  ...
  const comboItems = _comboItems as ComboItem[];
  const bottleItems = _bottleItems as BottleItem[];
  const table = _table as TableItem[];
  const vendor = _vendor as VendorInfo | undefined;
  ...
  const handleTableQuantityChange = (id?: string, change = 1, type = 'button') => {
    setTable(
      table.map((item) => {
        if (item._id === id) {
  ...
  const handleBottleQuantityChange = (id?: string, change = 1, type = 'button') => {
    setBottleItems(
  ...
  const handleComboSelectionChange = (id?: string) => {
    setComboItems(
      comboItems.map((item) => {
        if (item._id === id) {
  ...
  const getSelectedTablesText = () => {
    ...
    if (selectedTables.length === 1) {
      ...
      return t.quantity && t.quantity > 1 ? `${t.name} (${t.quantity})` : t.name || '';
    }
  ...
  const getTableDescription = (t: TableItem) => {
    ...
    return descriptions[t.category || ''] || 'Comfortable seating arrangement with great ambiance';
  };
        ...
        <div className="mb-6 space-y-6">
                                ...
                                <div className="flex items-center justify-between">
                                  ...
                                  <div className="flex items-center gap-2">
                                    ...
                                    <Button
                                      ...
                                      className="size-7 rounded-full border-[#0A6C6D] border text-[#0A6C6D] hover:bg-[#0A6C6D] hover:text-white"
                                      onClick={() => handleTableQuantityChange(item._id, 1)}
                                      disabled={(item.quantity || 0) >= available}
                                    >
          ...
          <div className="space-y-6">
            ...
            <div
              ...
              style={isMobile ? { scrollBehavior: 'smooth' } : undefined}
            >
              {!isMobile ? (
