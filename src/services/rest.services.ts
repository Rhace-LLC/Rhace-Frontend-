...
export const restaurantService = {
  async searchRestaurants(searchParams: string | Record<string, unknown> | null | undefined) {
    try {
      ...
      if (searchParams) {
        if (typeof searchParams === 'string') {
          ...
          const searchQuery = (searchParams.search || searchParams.query || '') as string;
          const type = (searchParams.type || searchParams.tab || 'restaurants') as string;
  ...
  async getSuggestions({ lat, lng }: { lat?: number | string; lng?: number | string }) {
    try {
...
export const dummyDrinks: DummyDrink[] = [
  {
    id: 1,
    name: 'Hennessy VS',
    category: 'Cognac',
