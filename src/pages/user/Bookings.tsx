import BookingCard from '@/components/updatedBookingCard';
...
function EmptyState({ type, isFiltered }: EmptyStateProps) {
  if (isFiltered) {
    return (
...
function buildSuggestions(bookings: any[]): Suggestion[] {
  const names = [...new Set(bookings.map((b) => b.vendor?.businessName).filter(Boolean))].map(
    (n: any) => ({ label: n, kind: 'vendor', value: n })
  );

  const types = [
    ...new Set(bookings.map((b) => normaliseType(b.reservationType)).filter(Boolean)),
  ].map((t: any) => ({ label: t, kind: 'type', value: t }));

  return [...names, ...types];
}

// Icon shown in the suggestion row
function SuggestionRowIcon({ kind, label }: { kind: string; label: string }) {
  if (kind === 'type') {
...
function SearchPanel({ query, onQueryChange, onClose, allBookings, onApply }: SearchPanelProps) {
        ...
        <div className="mb-10 flex items-center gap-3 text-gray-900">
          ...
          <div>
            ...
            <p className="text-xs sm:text-sm text-gray-400">
              View active, past and upcoming Reservations
            </p>
          </div>
        </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
                    {displayBookings.map((booking) => (
                      <BookingCard
                        key={booking._id}
                        booking={booking}
                        onEdit={(_id: string) => console.log('Edit', _id)}
