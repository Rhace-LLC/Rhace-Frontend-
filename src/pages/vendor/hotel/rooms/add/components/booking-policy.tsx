...
export function BookingPolicyForm({ onSubmit, formData, setFormData }: BookingPolicyProps) {
  const handleInputChange = (field: string, value: unknown) => {
    setFormData((prev) => ({ ...(prev ?? {}), [field]: value }));
  };

  const handlePaymentOptionChange = (option: string, checked: boolean | 'indeterminate') => {
    setFormData((prev) => ({
      ...(prev ?? {}),
      paymentOptions: {
        ...(((prev ?? {}) as Record<string, any>).paymentOptions ?? {}),
        [option]: !!checked,
      },
    }));
  };
