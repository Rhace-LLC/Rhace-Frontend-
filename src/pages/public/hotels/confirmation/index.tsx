  ...
  useEffect(() => {
    ...
    const processPayment = async () => {
      try {
        ...
        listDrafts('hotel').forEach((draft) => clearDraft(draft.id));
              ...
              data.rooms.map((item: any, index: number) => (
                <div key={index} className="py-2">
                  <div className="mb-2 text-xs text-medium">
                    Superion {item.roomId.category || item.roomId.roomCategory} {item.roomId.name}
                  </div>
                    ...
                    <div key={index} className="grid py-4 grid-cols-2 gap-4">
                      <div className="space-y-1">
                        ...
                        <p className="text-sm  line-clamp-1 font-medium text-gray-900">
                          Superion {room.roomId.category || room.roomId.roomCategory}{' '}
                          {room.roomId.name}
                        </p>
                      </div>
                      <div className="space-y-1">
