...
interface Reservation {
  ...
  meal?: { preselected?: boolean } | string;
  mealPreselected?: boolean;
  payment?: ReservationPayment | string;
...
export default function Reservations() {
  ...
  const [counters, setCounters] = useState<ReservationCounters>({
    todays: 0,
    prepaid: 0,
    expectedGuests: 0,
    pendingPayments: 0,
  ...
  const getReservationId = (res: Reservation) => res.id || res._id || res.reference;
  ...
  useEffect(() => {
    ...
    const handleReservationUpdate = (updatedReservation: Reservation) => {
      setReservations((prev) =>
        prev.map((r) =>
          getReservationId(r) === getReservationId(updatedReservation) ? updatedReservation : r
        )
      );
    };

    const handleReservationCreate = (newReservation: Reservation) => {
      setReservations((prev) => [newReservation, ...prev]);
    };

    const handleReservationDelete = (deletedReservation: Reservation) => {
      setReservations((prev) =>
        prev.filter((r) => getReservationId(r) !== getReservationId(deletedReservation))
      );
    };

    const handleCountersUpdate = (updatedCounters: CounterPayload) => {
      setCounters({
    ...
    const handlePaymentUpdate = (paymentUpdate: PaymentUpdatePayload) => {
      setReservations((prev) =>
        prev.map((r) => {
  ...
  const fetchCounters = async () => {
    try {
      ...
      const c = (countersRes?.data || {}) as Record<string, any>;
      setCounters({
  ...
  const handleEdit = (reservation: Reservation) => {
    ...
    setEditForm({
      ...
      paymentStatus: reservation.paymentStatus || (reservation.payment as ReservationPayment | undefined)?.status || '',
    });
                  ...
                  <tr key={i} className="border-b hover:bg-accent/50">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className="text-sm font-medium">
                            {res.customer || res.customerName || res.user?.name || 'Guest'}
                          </p>
                          <p className="text-xs text-muted-foreground">
                            ID: {res.id || res._id || res.reference}
                          </p>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="p-3">
                      <Badge
                        variant="secondary"
                        className={
                          res.meal === 'Yes' || res.mealPreselected || (res.meal as { preselected?: boolean } | undefined)?.preselected
                    ...
                    <td className="p-3">
                      <Badge
                        ...
                        className={
                          ...
                          (res.payment as ReservationPayment | undefined)?.status === 'paid'
                            ? 'bg-green-100 text-success'
                            : 'bg-red-100 text-destructive'
                        }
                      >
                        {String(res.payment || res.paymentStatus || (res.payment as ReservationPayment | undefined)?.status || '-')}
                      </Badge>
                    </td>
                    <td className="p-3">
                      <Badge
                    ...
                    <td className="p-3">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuItem onClick={() => handleCancel(res)}>
                            Cancel
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
          ...
          <div className="bg-white rounded-lg shadow-lg w-full max-w-md p-4">
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <select
                  className="w-full border rounded px-2 py-1"
                  value={filters.hasMeals}
                  onChange={(e) => setFilters((f) => ({ ...f, hasMeals: e.target.value }))}
                >
                  <option value="">Any</option>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm">
                    {selectedReservation.time || selectedReservation.checkInTime}
                  </p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <Badge variant="secondary">
                    {selectedReservation.status || selectedReservation.reservationStatus}
                  </Badge>
                </div>
                <div>
                  ...
                  <Badge variant="secondary">
                    {String(
                      selectedReservation.payment ||
                        ...
                        (selectedReservation.payment as ReservationPayment | undefined)?.status ||
                        '-'
                    )}
                  </Badge>
                </div>
                <div>
                  ...
                  <Badge variant="secondary">
                    {selectedReservation.meal === 'Yes' ||
                    selectedReservation.mealPreselected ||
                    (selectedReservation.meal as { preselected?: boolean } | undefined)?.preselected
                      ? '✓ Yes'
                      : '✗ No'}
                  </Badge>
                </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value) => setEditForm((f) => ({ ...f, paymentStatus: value }))}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
