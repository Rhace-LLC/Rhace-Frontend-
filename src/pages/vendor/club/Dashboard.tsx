...
const ClubDashboard = () => {
  ...
  const vendor = useSelector((state: any) => state.auth.vendor);
  ...
  const trendTotal = chartData.reduce((s: number, d: any) => s + d.thisWeek, 0);
  ...
  const generateDonutSegment = (count: number, total: number) => {
  ...
  const upcomingReservations = (reservationStats.todaysReservations || []).filter((r: any) => {
    if (!r.date || !r.time) return false;
    const dt = new Date(`${r.date.split('T')[0]}T${r.time}`);
    const diff = (Number(dt) - Number(new Date())) / 60000;
  ...
  const totalDrinkQty = drinks.reduce((s: number, d: any) => s + d.quantity, 0);

  // Combos — backend returns `count` (not quantity) for combos
  const combos = [...(reservationStats.clubCombosBreakdown || [])].sort(
    (a: any, b: any) => b.count - a.count
  );
  const totalComboCount = combos.reduce((s: number, c: any) => s + c.count, 0);
      ...
      <div className="min-h-screen bg-gray-50 p-2 md:p-6 mb-14">
        <div className="max-w-7xl mx-auto space-y-6">
            ...
            <div className="bg-yellow-50 border-l-4 border-yellow-400 rounded-lg p-4 flex items-center justify-between">
              <div className="flex items-center">
                ...
                <p className="text-yellow-800 text-sm font-medium">
                  {upcomingReservations.length} Booking{upcomingReservations.length > 1 ? 's' : ''}{' '}
                  commencing in the next 30 minutes
                </p>
              </div>
              <button
          ...
          <div className="grid grid-cols-1 bg-white md:grid-cols-2 lg:grid-cols-4 gap-4 rounded-lg border border-gray-200">
            {reservationStats.todayStats.map((stat: any, index: number) => {
            ...
            <div className="bg-white rounded-lg border border-gray-200">
              ...
              <div className="p-5 space-y-3">
                {reservationStats.todaysReservations.length > 0 ? (
                  reservationStats.todaysReservations.slice(0, 5).map((reservation: any) => (
                    <div
                      key={reservation._id}
                ...
                {drinks.length === 0 ? (
                  ...
                  <>
                    ...
                    <div className="space-y-3">
                      {drinks.slice(0, 5).map((item: any, i: number) => (
                        <div
                          key={i}
                        ...
                        >
                          <div className="flex items-center flex-1 min-w-0">
                            <div
                              className={`w-3 h-3 rounded-sm ${DRINK_COLORS[i % DRINK_COLORS.length]} mr-2 flex-shrink-0`}
                            />
                          ...
                          <div className="flex items-center gap-3 ml-4">
                            <span className="text-gray-600">
                              {totalDrinkQty > 0
                                ? ((item.quantity / totalDrinkQty) * 100).toFixed(1)
                                : 0}
                              %
                            </span>
              ...
              <div className="p-5">
                {combos.length === 0 ? (
                  ...
                  <>
                    ...
                    <div className="space-y-3">
                      {combos.slice(0, 5).map((item: any, i: number) => (
                        <div
                          key={i}
                          className="flex items-center justify-between text-sm hover:bg-gray-50 p-2 rounded transition-colors"
                        >
                          <div className="flex items-center flex-1 min-w-0">
                            <div
                              className={`w-3 h-3 rounded-sm ${COMBO_COLORS[i % COMBO_COLORS.length]} mr-2 flex-shrink-0`}
                            />
                            <span className="text-gray-900 font-medium truncate">
                              {item.comboName}
                            </span>
                          ...
                          <div className="flex items-center gap-3 ml-4">
                            <span className="text-gray-600">
                              {totalComboCount > 0
                                ? ((item.count / totalComboCount) * 100).toFixed(1)
                                : 0}
                              %
                            </span>
                            <span className="text-gray-900 font-medium min-w-[60px] text-right">
              ...
              {revenueData.items.length === 0 ? (
                <div className="h-40 flex flex-col items-center justify-center gap-2">
                  ...
                  <p className="text-xs text-gray-400">
                    Start taking paid bookings to see revenue breakdown
                  </p>
                </div>
              ) : (
                <>
                  <div className="mb-6">
                    ...
                    <p
                      className={`text-sm mt-1 flex items-center ${revenueData.change >= 0 ? 'text-green-600' : 'text-red-500'}`}
                    >
                  ...
                  <div className="flex h-4 rounded-full overflow-hidden mb-6">
                    {revenueData.items.map((item: any, i: number) => (
                      <div
                        key={i}
                  ...
                  <div className="relative w-48 h-48 mb-4">
                    <svg className="w-full h-full -rotate-90">
                      <circle
                        ...
                        strokeWidth="24"
                      />
                      {sourceData.sources.map((source: any, i: number) => {
                        const preceding = sourceData.sources
                          .slice(0, i)
                          .reduce((s: number, src: any) => s + src.count, 0);
