...
import { FavoriteButton3, type FavoriteVendor } from './favoritebutton';

interface SaveCopyProps {
  id?: string;
  type?: string;
  vendor?: FavoriteVendor;
}

const SaveCopy = ({ id, type, vendor }: SaveCopyProps) => {
  const handleCopy = () => {
    navigator.clipboard
      .writeText(`https://rhace-frontend.vercel.app/${type}/${id}`)
