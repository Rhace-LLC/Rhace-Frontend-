...
import { UserProfileMenu } from './UserHeader';
...
export default function Footer() {
  ...
  const navigate = (path: string) => navigates(path);

  useEffect(() => {
    const fetchUserData = async () => {
      try {
        setLoading(true);
        if (isAuthenticated) setProfile(user);
      } catch (error) {
        console.log(error);
      } finally {
        setLoading(false);
          ...
          <div className="relative z-10 max-w-7xl mx-auto px-10">
            ...
            <div className="pt-20 pb-14 border-b border-[#1e1e1e]">
              ...
              >
                Every reservation
                <br />
                <em className="not-italic" style={{ color: '#00a896' }}>
                  is a moment.
                </em>
              </motion.h2>
            </div>
            ...
            <div className="py-14 grid md:grid-cols-3 gap-16 border-b border-[#1e1e1e]">
              ...
              >
                <img
                  src={logoWhite}
                  alt="Rhace"
                  className="h-5 w-auto object-contain mb-5 opacity-70"
                />
                <p className="text-[13px] text-[#555] leading-relaxed font-light max-w-[200px]">
                  Discover, book, and enjoy moments that matter, accros Africa's best restaurants,
                  hotels, and clubs.
                </p>
              </motion.div>
              ...
              >
                <div>
                  <p className="text-[10px] uppercase tracking-[0.2em] text-[#3a3a3a] mb-5">
                    Explore
                  </p>
                  <ul className="space-y-3.5">
                    {[
                      ...
                      { label: 'Moments', href: '/bookings' },
                    ].map(({ label, href }) => (
                      <li key={label}>
                        <a
                          href={href}
                          className="text-[13px] text-[#666] hover:text-white transition-colors duration-200 font-light"
                        >
                          {label}
                        </a>
                      </li>
              ...
              <div className="flex items-center gap-6">
                {[
                  ...
                  <a
                    key={label}
                    href={href}
                    className="text-[12px] text-[#3a3a3a] hover:text-white transition-colors duration-200"
                  >
                    {label}
                  </a>
                ))}
