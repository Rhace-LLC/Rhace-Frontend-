...
import { AddRestaurantTableModal } from './components/AddRestaurantTableModal';
...
export default function ManageRestaurantTables() {
  ...
  const fetchTables = async () => {
    try {
      setIsLoading(true);
      const data = await restaurantTableService.getTables(vendor._id);
  ...
  const totalSeats = tables.reduce((sum, t) => sum + Number(t.seatingCapacity || 0), 0);
      ...
      <div className="min-h-screen bg-gray-50 p-2 md:p-6 mb-12">
        <div className="max-w-7xl mx-auto">
          <div className="md:flex justify-between items-center mb-6">
            <h2 className="text-[#111827] font-semibold mb-2">Table Management</h2>
                    ...
                    {paginatedItems.map((item) => (
                      <tr key={item._id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-4 text-sm text-gray-900 font-medium">
                          ₦{(item.minSpend || 0).toLocaleString()}
                        </td>
                        <td className="px-4 py-4">
                          <span
