...
export default function ReservationSummary() {
  ...
  const roomSelections: any[] = _roomSelections as any[];
  ...
  const handleUpdateQuantity = (roomId: string, delta: number) => {
    ...
    if (selection) {
      ...
      const newMaxAdults =
        (selection.room.adultsCapacity ?? selection.room.maxAdults ?? 2) * newQty;
      const newMaxChildren =
        (selection.room.childrenCapacity ?? selection.room.maxChildren ?? 2) * newQty;
  ...
  const handleUpdateDates = (roomId: string, field: string, value: Date) => {
    updateRoomSelection(roomId, { [field]: value } as any);
  };

  // Update guests for a room
  const handleUpdateGuests = (roomId: string, value: any, breakdown: any) => {
    updateRoomSelection(roomId, { guests: value, guestBreakdown: breakdown } as any);
  };
  if (loading) {
  ...
  return (
    <div className="min-h-screen mb-[65px] md:mt-0 bg-gray-50">
      ...
      <div className="md:hidden flex items-center gap-3 px-4 py-3 mt-4 ">
        <button onClick={() => (next === true ? showNext(false) : navigate(`/hotels/${id}`))}>
          <svg
            width="20"
            height="20"
            viewBox="0 0 20 20"
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-[12px] md:text-sm font-medium">
                    {vendor?.rating || '4.8'} ({vendor?.reviews?.toLocaleString() || '1,000'}{' '}
                    reviews)
                  </span>
                </div>
              </div>
                  ...
                  <div className="space-y-4 p-4">
                    {roomSelections && roomSelections.length > 0 ? (
                      roomSelections.map((selection) => {
                        const { room, checkInDate, checkOutDate, guests = 1 } = selection;
                        ...
                        const maxChildren =
                          (room.childrenCapacity ?? room.maxChildren ?? 2) * quantity;
                            ...
                            <div className="flex justify-between items-start mb-3">
                              <div className="flex-1">
                                ...
                                <p className="text-xs text-gray-500">
                                  {formatPrice(discountedPrice)}/night × {quantity} room × {nights}{' '}
                                  night
                                  {nights !== 1 ? 's' : ''}
                                </p>
                              </div>
                            ...
                            <div className="grid grid-cols-2 gap-2 mb-3">
                              <div>
                                <DatePicker
                                  ...
                                  value={checkInDate}
                                  onChange={(date) =>
                                    handleUpdateDates(room._id, 'checkInDate', date)
                                  }
                                  className="bg-white "
                                  edit
                                />
                              ...
                              <div>
                                <DatePicker
                                  ...
                                  value={checkOutDate}
                                  onChange={(date) =>
                                    handleUpdateDates(room._id, 'checkOutDate', date)
                                  }
                                  className="bg-white "
                                  edit
                                />
                            ...
                            <div className="flex items-center gap-2 justify-between">
                              <div className="flex w-full items-center gap-2">
                                <GuestPicker
                                  ...
                                  onChange={
                                    (value, breakdown) =>
                                      handleUpdateGuests(room._id, value, breakdown) // ← forward breakdown
                                  }
                                  className="bg-white"
                                  maxAdults={maxAdults}
                      ...
                      <div className=" divide-y">
                        ...
                        >
                          <div className="space-y-1">
                            ...
                            <p className="text-xs">
                              Pay {formatPrice(calculateTotalPrice() / 2)} now, and{' '}
                              {formatPrice(calculateTotalPrice() / 2)} on arrival. No extra fees
                            </p>
                          </div>
                          <svg
                            width="20"
                      ...
                      roomSelections.map((selection) => {
                        ...
                        return (
                          <div key={room._id} className="flex items-center justify-between">
                            ...
                            <p className="text-[#111827]">{formatPrice(roomTotal)}</p>
                          </div>
                        );
                      })}
                  </div>
