...
// ── Shared vendor shape used by search results & discovery ─────────────────────
export interface VendorLike {
  ...
  profileImages?: (string | { url?: string })[];
  image?: string;
  rating?: number;
...
const vendorType = (type?: string): 'restaurant' | 'hotel' | 'club' =>
  type === 'restaurant' || type === 'hotel' || type === 'club' ? type : 'restaurant';
...
export const VenueCard = ({ vendor, activeType }: VenueCardProps) => {
  const navigate = useNavigate();
  const type = vendorType(vendor.vendorType || activeType);
        ...
        <div className="flex justify-between items-start gap-2">
          <div className="min-w-0">
            ...
            {vendor.vendorTypeCategory && vendor.vendorTypeCategory !== 'General' && (
              <p className="text-xs text-gray-400 capitalize">{vendor.vendorTypeCategory}</p>
            )}
          </div>
...
export const DiscoveryListCard = ({ vendor, activeType, navigate: nav }: DiscoveryListCardProps) => {
  ...
  const ctaLabel = type === 'hotel' ? 'Book Stay' : type === 'club' ? 'Book now' : 'Reserve now';
  ...
  return (
    ...
    >
      ...
      {/* Info */}
      <div className="flex-1 min-w-0" onClick={handleClick} style={{ cursor: 'pointer' }}>
        ...
        <p className="text-[11px] text-gray-400 mt-0.5 line-clamp-1 capitalize leading-tight">
          {isClosed ? (
            <span className="text-red-400 font-medium">Closed until tomorrow</span>
          ) : (
            description
          )}
        </p>
...
const Shimmer = ({ className = '' }: ShimmerProps) => <div className={className} style={shimmerStyle} />;
