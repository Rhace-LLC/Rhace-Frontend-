...
export const QuickFilterChips = ({ activeType, filters, onChange }: QuickFilterChipsProps) => {
    ...
    <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-1 mb-3">
      {chips.map((chip) => {
        const isActive = chip.multi
          ? ((filters[chip.key] as string[]) || []).includes(chip.val)
        ...
        return (
          <button
            ...
            onClick={() => {
              if (chip.multi) {
                const arr: string[] = (filters[chip.key] as string[]) || [];
                onChange(
                  chip.key,
                  isActive ? arr.filter((v) => v !== chip.val) : [...arr, chip.val]
                );
              } else {
