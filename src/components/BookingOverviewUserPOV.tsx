...
const paymentModeMap: Record<string, { label: string }> = {
...
function formatReservationType(type: string | undefined) {
  if (!type) return '';
...
function getReservationIcon(type: string | undefined) {
...
function HotelDetails({ booking }: { booking: ReservationData }) {
...
function RestaurantDetails({ booking }: { booking: ReservationData }) {
  return (
    <>
            ...
            return (
              <div key={i} className="sm:col-span-2 bg-gray-50 rounded-lg p-3 flex items-center justify-between">
                ...
                <div className="text-right">
                  <p className="text-sm font-semibold">{formatCurrency(menuData.price || 0)}</p>
...
function ClubDetails({ booking }: { booking: ReservationData }) {
  return (
    <>
      <Section title="Reservation Schedule">
        <InfoRow
          ...
          {booking.drinks?.map((d, i) => {
            ...
            return (
              <div key={i} className="bg-gray-50 rounded-lg p-3 flex items-center justify-between">
                ...
                <div className="text-right">
                  <p className="text-sm font-semibold">{formatCurrency(drinkData.price || 0)}</p>
...
function PaymentDetails({ booking, onPayBalance }: { booking: ReservationData; onPayBalance?: () => void }) {
            ...
            return (
              <div key={payment._id} className="rounded-lg border border-gray-200 p-3 space-y-2">
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
                  ...
                  <span className="font-medium text-right">{formatCurrency(payment.amountPaid || 0)}</span>
...
interface BookingOverviewUserPOVProps {
...
function BookingOverviewUserPOV({ booking, open, onOpenChange, onPayBalance }: BookingOverviewUserPOVProps) {
