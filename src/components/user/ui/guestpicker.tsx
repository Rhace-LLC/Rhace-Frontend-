...
const GUEST_CONFIG: Record<string, { label: string; subtitle: string; min: number }> = {
...
const totalGuests = (c: GuestCounts) => c.adults + c.children + c.infants;
...
export function GuestPicker({
  ...
  const isDisabled = (type: string, val: number, current: GuestCounts = counts) => {
    if (type === 'adults') {
    ...
    if (type === 'infants') {
      ...
      const maxWithoutInfants = (maxAdults ?? Infinity) + (maxChildren ?? Infinity);
  ...
  const inc = (type: string) => {
    setCounts((c) => {
      const val = c[type as keyof GuestCounts];
  ...
  const dec = (type: string) => {
    setCounts((c) => {
      ...
      const next = { ...c, [type]: Math.max(c[type as keyof GuestCounts] - 1, min) };
      onChange?.(totalGuests(next), next); // ← pass breakdown as 2nd arg
      return next;
    });
  };
      ...
      <PopoverContent className="w-72 mt-2 overflow-auto">
        {Object.keys(GUEST_CONFIG)
          ...
          .map((type) => {
            ...
            const vals = counts[type as keyof GuestCounts];
            return (
              <div
                key={type}
              ...
              >
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div className="outline-1 h-8 px-3 outline-offset-[-1px] outline-neutral-200 inline-flex items-center justify-center">
                    <span className=" text-center font-medium text-sm text-gray-700 ">{vals}</span>
                  </div>
                  <button
                    onClick={() => {
                      inc(type);
