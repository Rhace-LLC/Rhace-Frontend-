...
interface StatsState {
  ...
  payments: {
    completed: { thisWeek: number; lastWeek: number; change: number };
    pending: { thisWeek: number; lastWeek: number; change: number };
  };
}
...
interface InfoState {
  ...
  lastPayment?: string | number | Date;
  [key: string]: unknown;
}
...
const PaymentDashboard = () => {
  ...
  const getPaymentStatus = (payment: PaymentRow) => {
    const status = (payment?.paymentStatus || payment?.payment_status || payment?.status || '')
      .toLowerCase()
      .trim();
  ...
  const isSuccess = (status: string) => ['success', 'paid', 'completed'].includes(status.toLowerCase());
  const isPending = (status: string) => ['pending', 'processing'].includes(status.toLowerCase());

  const computeClientStats = (payments: PaymentRow[]) => {
    const now = new Date();
    const oneWeekAgo = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000); // 7 days ago

    const thisWeekPayments = payments.filter((p) => new Date(p.createdAt as Date) >= oneWeekAgo);
  ...
  const columns: ColumnDef<PaymentRow, any>[] = [
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <span className="text-[#111827] font-medium text-sm">{formatDate(user.createdAt as Date)}</span>
        );
      },
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex items-center gap-3">
            <Avatar>
              <AvatarFallback>
                {user.customerName!.split(' ').map((i: string) => i.slice(0, 1).toUpperCase())}
              </AvatarFallback>
            </Avatar>
    ...
    const fetchPayments = async () => {
      try {
        ...
        const revenue = res
          ...
          .reduce((sum: number, item: PaymentRow) => sum + (item.amountPaid || item.amount || 0), 0);
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 py-4 md:py-6 border w-full bg-white rounded-2xl md:divide-x divide-y md:divide-y-0 [&>div]:pb-0 md:[&>div]:px-2">
                  <div className="flex h-full items-center">
                    <StatCard
                      ...
                      className="py-0"
                      value={`₦${stats.earnings.thisYear.toLocaleString('en-US', {
                  ...
                  <div className="flex h-full items-center">
                    <StatCard
                      ...
                      className="py-0"
                      value={`₦${stats.earnings.thisWeek.toLocaleString('en-US', {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2,
                      })}`}
                  ...
                  <div className="flex h-full items-center">
                    <StatCard
                      ...
                      className="py-0"
                      value={`₦${stats.payments.completed.thisWeek.toLocaleString('en-US', {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2,
                      })}`}
                  ...
                  <div className="flex h-full items-center">
                    <StatCard
                      ...
                      className="py-0"
                      value={`₦${stats.payments.pending.thisWeek.toLocaleString('en-US', {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2,
                      })}`}
