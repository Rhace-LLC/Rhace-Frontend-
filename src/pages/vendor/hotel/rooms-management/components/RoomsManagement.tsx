...
interface RoomsManagementComponentProps {
  currentPage?: number;
  itemsPerPage?: number;
...
const RoomsManagementComponent = ({
  ...
  const filterRef = useRef<RoomFilterHandle | null>(null);

  const dispatch = useDispatch<AppDispatch>();
  const roomTypesState = useSelector(selectRoomTypes);
  const vendor = useSelector((state: RootState) => state.auth.vendor) as any;
