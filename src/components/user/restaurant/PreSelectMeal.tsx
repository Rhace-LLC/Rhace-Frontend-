...
interface PreSelectMealProps {
  id: string;
}

export default function PreSelectMeal({ id }: PreSelectMealProps) {
  const {
    activeTab,
    setActiveTab,
    additionalNote,
  ...
  const menuFiltered = (menuItems as MenuCardItem[]).filter((a) => a.category === activeTab);
  ...
  const selectedItems = (menuItems as MenuCardItem[]).filter((item) => item.selected);
      ...
      <div className="bg-[#E7F0F0] border-[#B3D1D2] sticky top-0 z-50 w-full">
        <div className="max-w-4xl mx-auto rounded-md mb-6 p-4">
          ...
          <div className="flex flex-wrap items-center gap-y-1 text-xs md:text-sm">
            <span className="font-medium">{(vendor as { businessName?: string } | undefined)?.businessName || 'Restaurant Name'}</span>
            ...
            <Button
              ...
              onClick={() =>
                setVisibleCount((prev) => {
                  ...
                  const filteredLength = (menuItems as MenuCardItem[]).filter(
