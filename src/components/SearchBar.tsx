...
import { TYPE_CONFIG, type VendorTypeKey } from '../utils/constants';
import { delRecent, getRecent } from '../utils/filterUtils';
...
export const SearchBar = ({
  ...
  const typeKey = (filters.type || 'restaurant') as VendorTypeKey;
    ...
    <div className="flex-1 relative">
      ...
      >
        ...
        <input
          ...
          placeholder={
            filters.type
              ? TYPE_CONFIG[typeKey]?.searchPlaceholder
...
// ── Shared dropdown row ───────────────────────────────────────────────────────
interface VendorDropdownRowProps {
  vendor: SearchSuggestion;
  onClick: () => void;
  showArrow?: boolean;
}

const VendorDropdownRow = ({ vendor, onClick, showArrow }: VendorDropdownRowProps) => {
  const cfg = TYPE_CONFIG[(vendor.vendorType || 'restaurant') as VendorTypeKey];
  return (
    <div
      onMouseDown={(e) => e.preventDefault()}
      onClick={onClick}
...
export const SearchPopup = ({ show, setShow, searchProps, filters, inputRef }: SearchPopupProps) => {
  useEffect(() => {
    if (show) {
            ...
            {searchProps.suggestions.length > 0 && (
              <>
                {searchProps.isSugLoading ? (
                  ...
                  <p className="px-3 py-4 text-center text-sm text-gray-400">
                    No results for "{searchProps.inputValue}"
                  </p>
                )}
              </>
            )}
