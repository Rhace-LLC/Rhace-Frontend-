...
const Header = ({ onClick = () => {}, activeTab = null }: HeaderProps) => {
  ...
  const renderAuthButtons = () => {
    if (loading) {
      return <div className="size-10 bg-gray-300 mr-[50px] animate-pulse rounded-full" />;
    }
    if (profile) {
      return (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-72 bg-gray-50 rounded-2xl">
            ...
            <DropdownMenuItem asChild className="  px-4 py-2">
              <Link to="/payments">Payments/Transaction</Link>
            </DropdownMenuItem>
    ...
    return (
      <>
        ...
        <Button className="cursor-pointer rounded-full bg-teal-700 hover:bg-teal-700/90" asChild>
          <Link to="/auth/user/signup">Create Account</Link>
        </Button>
      </>
    );
  };

  const SvgIcon = ({ isActive }: { isActive?: boolean }) => (
  ...
  const SvgIcon2 = ({ isActive }: { isActive?: boolean }) => (
  ...
  const SvgIcon3 = ({ isActive }: { isActive?: boolean }) => (
          ...
          <div className="flex">
            <div className="shrink-0 flex items-center">
              <Link to="/" className="flex items-center space-x-2">
                <img
                  src={scrolled && isHomePage ? logoBlack : isHomePage ? logoWhite : logoBlack}
                  alt="Rhace Logo"
                  ...
                  >
                    ...
                    <span
                      className={`absolute h-0.5 w-0 bg-teal-500 left-1/2 -translate-x-1/2 bottom-0 rounded-full
                          ${
                            isActive ? 'w-[24px] h-2' : 'group-hover:w-[24px] h-2'
