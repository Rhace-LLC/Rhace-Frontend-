...
const loadPaystackStats = async (setPaystackStats, setPaystackBalance, setPaystackError) => {
  try {
    ...
    const weeklyChange =
      lastWeekEarnings > 0 ? ((weeklyEarnings - lastWeekEarnings) / lastWeekEarnings) * 100 : 0;
    const todayChange =
      yesterdayEarnings > 0 ? ((todayEarnings - yesterdayEarnings) / yesterdayEarnings) * 100 : 0;
    const completedChange =
      lastWeekCount > 0 ? ((completedPayments - lastWeekCount) / lastWeekCount) * 100 : 0;
    setPaystackStats({
      totalEarnings,
      weeklyEarnings,
      weeklyEarningsChange: weeklyChange,
      weeklyEarningsTrend: weeklyChange >= 0 ? 'up' : 'down',
      todayEarnings,
      todayEarningsChange: todayChange,
      todayEarningsTrend: todayChange >= 0 ? 'up' : 'down',
      completedPayments,
      completedPaymentsChange: completedChange,
      completedPaymentsTrend: completedChange >= 0 ? 'up' : 'down',
      loading: false,
      lastUpdated: new Date(),
    });
  } catch (error) {
    ...
    >
            ...
            {miniSparkData.map((v, i) => (
              <div
                ...
                style={{
                  flex: 1,
                  height: `${(v / 100) * 20}px`,
                  background: i === miniSparkData.length - 1 ? '#6366f1' : 'rgba(99,102,241,0.25)',
                  borderRadius: '2px',
      ...
      >
        ...
        >
          ...
          <p style={{ margin: '0 0 2px', fontSize: '11px', color: '#475569', fontWeight: 500 }}>
            Available
          </p>
...
export default function Payments() {
  ...
  const [dashboardKPIs, setDashboardKPIs] = useState<Record<string, any>>({});
  ...
  const fetchPaystackStats = async () => {
    await loadPaystackStats(setPaystackStats, setPaystackBalance, setPaystackError);
  };
    ...
    try {
      ...
      setDashboardKPIs((prev) => ({
        ...
        totalPaystackCommission:
          data.summary?.totalPaystackCommission || prev.totalPaystackCommission,
      }));
    } catch (error) {
  ...
  const fetchPayoutStats = async () => {
    try {
      ...
      const pendingData =
        pendingRes.status === 'fulfilled' ? extractArray(pendingRes.value?.data) : [];
      const successfulData =
        successfulRes.status === 'fulfilled' ? extractArray(successfulRes.value?.data) : [];
      ...
      setDashboardKPIs((prev) => ({
        ...prev,
        totalPayouts,
        totalPayoutsChange: '+0% from last year',
        totalPayoutsTrend: 'up',
        pendingPayouts,
        pendingPayoutsChange: '+0% vs last week',
        pendingPayoutsTrend: 'up',
        successfulPayouts: successfulPayoutsCount,
        successfulPayoutsChange: '+0% vs last week',
        successfulPayoutsTrend: 'up',
        lastPayout,
        lastPayoutChange: '+0% vs last week',
        lastPayoutTrend: 'up',
      }));
    } catch (error) {
    ...
    try {
      ...
      const enrichedPayouts = (Array.isArray(list) ? list : []).map((payout) => {
        ...
        const vendor = currentVendors.find(
          (v) => v.id === vendorId || v._id === vendorId || v.vendorId === vendorId
        );
        ...
        if (
          ...
          payout.vendor &&
          typeof payout.vendor === 'string' &&
          !/^[a-f0-9]{24}$/i.test(payout.vendor)
        ) {
    ...
    try {
      ...
      setDashboardKPIs((prev) => {
        ...
        Object.keys(kpis).forEach((key) => {
          if (kpis[key] !== undefined && !earningsKeys.includes(key)) updated[key] = kpis[key];
        });
        return updated;
      });
    } catch (e) {}
    ...
    try {
      ...
      const vendorsWithEarnings = earnings.map((vendor) => {
        ...
        const totalPaid = Number(
          vendor.totalPaid || vendor.paid || vendor.totalPayments || vendor.commissionPaid || 0
        );
  ...
  const handleInitiatePayout = async (vendorId, amount) => {
    try {
      if (!bankDetails.accountNumber || bankDetails.accountNumber.includes('•')) {
        alert(
          'Please update your bank account details with a valid account number before initiating a payout.'
        );
        return;
      }
  ...
  const handleAddAccount = () => {
    setTempAddDetails({ bankName: '', accountNumber: '', accountName: '' });
      ...
      try {
        ...
        if (!ignore) {
          ...
          const filteredKpis = Object.keys(kpis).reduce((acc, key) => {
            if (!earningsKeys.includes(key)) acc[key] = kpis[key];
            return acc;
          }, {});
  ...
  useEffect(() => {
    ...
    const lastPayment =
      payoutHistory.length > 0
        ? payoutHistory.reduce((latest, payout) => {
  ...
  useEffect(() => {
    if (!paystackStats.loading) {
      setDashboardKPIs((prev) => ({
        ...
        weeklyEarningsChange:
          paystackStats.weeklyEarningsChange !== undefined
            ? `+${paystackStats.weeklyEarningsChange.toFixed(1)}% vs last week`
            : prev.weeklyEarningsChange,
        weeklyEarningsTrend: paystackStats.weeklyEarningsTrend || prev.weeklyEarningsTrend,
        todayEarnings: paystackStats.todayEarnings,
        todayEarningsChange:
          paystackStats.todayEarningsChange !== undefined
            ? `+${paystackStats.todayEarningsChange.toFixed(1)}% vs yesterday`
            : prev.todayEarningsChange,
        ...
        completedPaymentsChange:
          paystackStats.completedPaymentsChange !== undefined
            ? `+${paystackStats.completedPaymentsChange.toFixed(1)}% vs last week`
            : prev.completedPaymentsChange,
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 w-full">
              <StatCard
                ...
                value={
                  dashboardKPIs.totalEarnings
                    ? `₦${dashboardKPIs.totalEarnings.toLocaleString()}`
                    : '₦0'
                }
              ...
              <StatCard
                ...
                value={
                  dashboardKPIs.weeklyEarnings
                    ? `₦${dashboardKPIs.weeklyEarnings.toLocaleString()}`
                    : '₦0'
                }
              ...
              <StatCard
                ...
                value={
                  dashboardKPIs.completedPayments
                    ? dashboardKPIs.completedPayments.toLocaleString()
                    : '0'
                }
              ...
              <StatCard
                ...
                value={
                  dashboardKPIs.todayEarnings
                    ? `₦${dashboardKPIs.todayEarnings.toLocaleString()}`
                    : '₦0'
                }
              ...
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <h3 className="text-2xl md:text-3xl font-bold mt-1">
                    {dashboardKPIs.availableBalance
                      ? `₦${dashboardKPIs.availableBalance.toLocaleString()}`
                      : '₦0'}
                  </h3>
                  <p className="text-xs text-muted-foreground mt-1">
                    Last payment processed on{' '}
                    {dashboardKPIs.lastPaymentDate
                      ? new Date(dashboardKPIs.lastPaymentDate).toLocaleDateString()
                      : 'N/A'}
                  </p>
                </div>
              </div>
                  ...
                  <div className="grid gap-4 py-4">
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={(value) =>
                          setTempBankDetails({ ...tempBankDetails, bankName: value })
                        }
                      >
                        <SelectTrigger>
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setTempBankDetails({ ...tempBankDetails, accountName: e.target.value })
                        }
                ...
                <TableBody>
                  {loadingEarnings ? (
                    ...
                    <TableRow>
                      ...
                      >
                        No vendors with earnings found
                      </TableCell>
                    </TableRow>
                  ) : (
        ...
        <TabsContent value="history" className="space-y-6 mt-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <StatCard
              title="Total Payouts"
              value={
                dashboardKPIs.totalPayouts
                  ? `₦${dashboardKPIs.totalPayouts.toLocaleString()}`
                  : '₦0'
              }
            ...
            <StatCard
              ...
              value={
                dashboardKPIs.pendingPayouts
                  ? `₦${dashboardKPIs.pendingPayouts.toLocaleString()}`
                  : '₦0'
              }
            ...
            <StatCard
              ...
              value={
                dashboardKPIs.successfulPayouts
                  ? `₦${dashboardKPIs.successfulPayouts.toLocaleString()}`
                  : '₦0'
              }
            ...
            <StatCard
              title="Last Payout"
              value={
                dashboardKPIs.lastPayout ? `₦${dashboardKPIs.lastPayout.toLocaleString()}` : '₦0'
              }
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Badge
                    variant={
                      getVendorStatus(selectedVendor.status) === 'Paid' ? 'default' : 'secondary'
                    }
                    className="mt-1"
                  >
