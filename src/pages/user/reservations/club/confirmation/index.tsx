...
export default function ConfirmPage() {
  ...
  useEffect(() => {
    ...
    let isMounted = true;
    let timeoutId: ReturnType<typeof setTimeout>; // We use timeout, not interval
    let pollCount = 0;
    const MAX_POLLS = 10;
    ...
    const completeReservation = async () => {
      try {
        ...
        listDrafts('club').forEach((draft) => clearDraft(draft.id));
          ...
          <div className="space-y-4 px-4 pb-4">
              ...
              <div className="space-y-4 px-2 py-2 rounded-2xl border border-[#E5E7EB] bg-[#F9FAFB]">
                ...
                <div className="flex justify-between items-end gap-3">
                  <div className="grid md:flex line-clamp-1 grid-cols-2 grid-rows-2 gap-x-4 gap-y-1">
                    {data.combos.slice(0, 4).map((item: any) => (
                  ...
                  <p className="text-sm font-medium text-[#111827] whitespace-nowrap">
                    ₦{data.combos.reduce((sum: number, i: any) => sum + (i.setPrice || 0), 0).toLocaleString()}
                  </p>
                </div>
              </div>
            )}
            ...
            {data.tables.length > 0 && (
              <div className="space-y-4 px-2 py-2 rounded-2xl border border-[#E5E7EB] bg-[#F9FAFB]">
                ...
                <div className="flex justify-between items-end gap-3">
                  <div className="grid md:flex line-clamp-1 grid-cols-2 grid-rows-2 gap-x-4 gap-y-1">
                    {data.tables.slice(0, 4).map((item: any) => (
                      <span key={item._id} className="text-xs text-[#111827] truncate">
                        {item.quantity > 0 && (
                  ...
                  <p className="text-sm font-medium text-[#111827] whitespace-nowrap">
                    ₦
                    {data.tables
                      .reduce((sum: number, i: any) => sum + (i.tableType.price || 0) * (i.quantity || 1), 0)
                      .toLocaleString()}
                  </p>
                </div>
              </div>
            ...
            {data.drinks.length > 0 && (
              <div className="space-y-4 px-2 py-2 rounded-2xl border border-[#E5E7EB] bg-[#F9FAFB]">
                ...
                <div className="flex justify-between items-end gap-3">
                  <div className="grid md:flex line-clamp-1 grid-cols-2 grid-rows-2 gap-x-4 gap-y-1">
                    {data.drinks.slice(0, 4).map((item: any) => (
                      <span key={item._id} className="text-xs text-[#111827] truncate">
                        {item.quantity > 0 && (
                          <span className="font-medium">{item.quantity}x </span>
                        )}
                  ...
                  <p className="text-sm font-medium text-[#111827] whitespace-nowrap">
                    ₦
                    {data.drinks
                      .reduce((sum: number, i: any) => sum + (i.drink.price || 0) * (i.quantity || 0), 0)
                      .toLocaleString()}
                  </p>
                </div>
              </div>
