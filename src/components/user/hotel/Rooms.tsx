...
const Rooms = ({ setSelectedRooms, setShow, rooms }: RoomsProps) => {
  const tabs: RoomTab[] = [
  ...
  const [activeTab, setActiveTab] = useState<RoomTab>(tabs[0]);
  ...
  const [selectedRooms, setSelectedRoomsLocal] = useState<HotelRoom[]>(() => {
  ...
  const syncToParent = (roomsData: HotelRoom[]) => {
  ...
  const handleDateChange = (roomId: string, field: string, value: Date) => {
    setRoomDetails((prev) => ({
      ...prev,
      [roomId]: {
        ...prev[roomId],
  ...
  const handleGuestsChange = (roomId: string, value: number) => {
    setRoomDetails((prev) => ({
      ...prev,
      [roomId]: {
        ...prev[roomId],
  ...
  const calculateNights = (roomId: string) => {
    const details = roomDetails[roomId] || getDefaultDates();
    const { checkInDate, checkOutDate } = details;
    if (!checkInDate || !checkOutDate) return 1;

    const msPerDay = 1000 * 60 * 60 * 24;
    return Math.max(1, Math.ceil((new Date(checkOutDate).getTime() - new Date(checkInDate).getTime()) / msPerDay));
  };

  // Add room to selection with quantity, dates, and guests
  const addToSelection = (room: HotelRoom) => {
  ...
  const removeFromSelection = (roomId: string) => {
  ...
  const toggleAmenities = (roomId: string, e: React.MouseEvent) => {
    e.stopPropagation();
   ...
   */
  const getAmenitiesList = (room: HotelRoom) => {
