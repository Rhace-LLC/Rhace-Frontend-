...
const TYPE_CONFIG: Record<string, { label: string; singular: string; path: string }> = {
  restaurant: {
    label: 'Restaurants',
    singular: 'Restaurant',
    path: 'restaurants',
...
const normalizeSuggestion = (item: unknown, fallbackType?: string): SearchItem | null => {
  ...
  return {
    _id: (it._id as string) || (it.id as string) || (it.businessName as string) || (it.name as string) || '',
    businessName:
      (it.businessName as string) || (it.name as string) || (it.title as string) || (it.value as string) || '',
    vendorType: (it.vendorType as string) || (it.type as string) || fallbackType || 'restaurant',
    profileImages: (it.profileImages as SearchItem['profileImages']) || (it.images as string[]) || [],
...
const searchSvc = {
  suggestions: async (q: string, type?: string, location?: UserLocation): Promise<SearchItem[]> => {
    if (!q) return [];
  ...
  trending: (type?: string): Promise<SearchItem[]> =>
    api
      .get(`/search/trending${type ? `?type=${type}` : ''}`)
      .then((r) => (r.data?.trending as SearchItem[]) || []),
};
        ...
        <div>
          ...
          {isTrendLoad ? (
            ...
            trending.slice(0, 6).map((v) => {
              const cfg = TYPE_CONFIG[v.vendorType || 'restaurant'];
...
const buildSearchUrl = ({ q, type, date, time, guests }: BuildSearchUrlParams) => {
  const params = new URLSearchParams();
  if (q) params.set('q', q);
  if (type) params.set('type', type);
...
type ActiveTab = 'restaurants' | 'hotels' | 'clubs';
...
interface SearchSectionProps {
  activeTab: ActiveTab;
  onSearch?: (data: { query?: string; tab?: string; date?: Date | null; time?: string | null; guests?: number }) => void;
}
...
const SearchSection = ({ activeTab, onSearch }: SearchSectionProps) => {
  ...
  const timeSlots: Record<ActiveTab, string[]> = {
    restaurants: [
      '09:00 AM',
      '09:30 AM',
      '10:00 AM',
...
export const SearchSectionTwo = ({ onSearch, searchData, activeTab }: SearchSectionTwoProps) => {
  const [date, setDate] = useState<Date | null>(null);
  const [time, setTime] = useState<string | null>(null);
  const [guests, setGuests] = useState<GuestCounts>({ adults: 2, children: 0, infants: 0 });
  const [inputValue, setInputValue] = useState('');
  ...
  const wrapperRef = useRef<HTMLDivElement>(null);
  const vendorType = TAB_TO_TYPE[activeTab || 'restaurants'] || 'restaurant';
  const userLocation = getUserLocation();
  ...
  useEffect(() => {
    ...
    if (!src) return;
    if (src.query) setInputValue(String(src.query));
    if (src.date) setDate(new Date(String(src.date)));
    if (src.time) setTime(String(src.time));
    if (src.guests)
      setGuests({
        adults: parseInt(String(src.guests), 10) || 2,
        children: 0,
        infants: 0,
      });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    const handle = (e: MouseEvent | TouchEvent) => {
      ...
      const outsideForm = formRef.current && !formRef.current.contains(target);
