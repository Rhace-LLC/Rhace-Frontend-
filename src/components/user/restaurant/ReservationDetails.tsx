...
export default function ReservationDetails({ id, searchQuery }: ReservationDetailsProps) {
  const {
    selectedOccasion,
    setSelectedOccasion,
    seatingPreference,
  ...
  const vendorInfo = vendor as ReservationVendor | undefined;
        ...
        <div className="max-w-[500px]">
          <div className="flex gap-4">
            <div className="relative size-16 md:w-32 md:h-24 rounded-2xl overflow-hidden shrink-0">
              <img
                src={(vendorInfo?.profileImages?.[0] as string) || '/hero-bg.png'}
            ...
            <div className="flex-1">
              <h2 className="text-sm md:text-xl font-semibold mb-2">
                {vendorInfo?.businessName || 'Restaurant Name'}
              </h2>
              <div className="flex items-start gap-1 text-gray-600 mb-2">
                ...
                <span className="text-[12px] md:text-sm truncate w-[210px] sm:w-full">
                  {vendorInfo?.address || '123 Main St, City, Country'}
                </span>
              </div>
              <div className="flex items-center gap-1">
                <Star className="h-4 w-4 fill-[#F0AE02] text-[#F0AE02]" />
                <span className="text-[12px] md:text-sm font-medium">
                  {vendorInfo?.rating || '4.8'} ({vendorInfo?.reviews.toLocaleString() || '1,000'}{' '}
                  reviews)
                </span>
              </div>
            </div>
