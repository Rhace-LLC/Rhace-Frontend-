...
const BottleServiceManager = () => {
  ...
  const addDrink = (drink) => {
    ...
    if (existing) {
      setSelectedDrinks(
        selectedDrinks.map((d) => (d._id === drink._id ? { ...d, quantity: d.quantity + 1 } : d))
      );
    } else {
  ...
  const updateQuantity = (id: string, delta: number) => {
    setSelectedDrinks(
      selectedDrinks
        .map((d) => (d._id === id ? { ...d, quantity: Math.max(1, d.quantity + delta) } : d))
        .filter((d) => d.quantity > 0)
    );
  };

  const _removeDrink = (id: string) => {
  ...
  const renderStep1 = () => (
    <div className="lg:flex gap-6 ">
      <div
        className={cn(
          selectedDrinks.length > 0 && 'lg:w-2/3 ',
          'lg:w-1/2 mx-auto mb-24 space-y-4 sm:space-y-6 px- py-4 sm:py-6'
        )}
      >
        {/* Club Identity */}
            ...
            <>
              ...
              <div className="space-y-3 max-h-96 overflow-y-auto">
                {filteredDrinks.map((drink) => (
                  <div
                    key={drink._id}
                    className="flex items-center justify-between p-3 border border-gray-200 rounded-lg hover:border-teal-500 transition-colors"
                  >
        ...
        <div
          className={`bg-white rounded-lg shadow-sm p-4 sm:p-6 mt-4 ${selectedTab === 'existing' ? 'block' : 'hidden'}`}
        >
          <label className="border-2 border-dashed border-gray-300 rounded-lg p-6 sm:p-8 text-center hover:border-teal-500 transition-colors cursor-pointer block">
            <input
              type="file"
        ...
        <div className="mt-4 sm:mt-6 flex items-center justify-between p-3 sm:p-4 bg-gray-50 rounded-lg gap-3">
          <div className="flex-1 min-w-0">
            <h3 className="font-medium text-gray-900 text-xs sm:text-base">
              Enable Price Visibility To Customers
            </h3>
            <p className="text-xs text-gray-500 mt-1">
              When disabled, customers will see "Contact for Price"
            </p>
          </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b px-4 py-3 lg:hidden">
        <div className="flex items-center justify-between text-sm">
          ...
          <div className="flex gap-2">
            <div
              className={`w-16 h-1 rounded-full ${currentStep === 1 ? 'bg-teal-600' : 'bg-teal-300'}`}
            ></div>
            <div
              className={`w-16 h-1 rounded-full ${currentStep === 2 ? 'bg-teal-600' : 'bg-gray-300'}`}
            ></div>
          </div>
        </div>
      </div>
      ...
      {/* Main Content */}
      <div className="min-h-screen">{currentStep === 1 ? renderStep1() : renderStep2()}</div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 sm:py-4">
          <div className="flex justify-between items-center gap-3 sm:gap-4">
            <button
              ...
              className={`px-4 sm:px-6 py-2 font-medium rounded-lg transition-colors text-sm sm:text-base ${
                currentStep === 1
