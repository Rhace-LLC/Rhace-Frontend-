...
function CardHeader({ iconKey, title, sub, badge }: CardHeaderProps) {
  return (
...
function BtnRow({ onCancel, onSave, label }: BtnRowProps) {
  return (
...
interface AvatarEditorProps {
  ...
  firstName: string;
  lastName: string;
  onAvatarChange: (payload: AvatarPayload | null) => void;
  uploadRef: React.RefObject<(() => void) | null>;
}

function AvatarEditor({ avatar, firstName, lastName, onAvatarChange, uploadRef }: AvatarEditorProps) {
...
export default function AccountSettings() {
  ...
  const [pwHint, setPwHint] = useState<{ msg: string; type: 'muted' | 'error' | 'success' }>({
    msg: '',
    type: 'muted',
  });
  const [toast, setToast] = useState<any>({ visible: false, msg: '' });
  ...
  const matchHint = (): { msg: string; type: 'muted' | 'error' | 'success' } =>
    !confirmPw
      ? { msg: '', type: 'muted' }
      : newPw === confirmPw
  ...
  const handleAvatarChange = async (payload: AvatarPayload | null) => {
    // Handle remove
    if (!payload) {
      setAvatar(null);
      try {
  ...
  const eyeBtn = (show: boolean, setShow: React.Dispatch<React.SetStateAction<boolean>>) => (
    <button
