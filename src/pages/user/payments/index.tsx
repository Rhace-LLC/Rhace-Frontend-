...
const STATUS: Record<'Paid' | 'Pending' | 'Failed', StatusConfig> = {
  Paid: {
    label: 'Paid',
    bg: 'bg-emerald-50',
...
const normaliseStatus = (s?: string | null): 'Paid' | 'Pending' | 'Failed' => {
  if (!s) return 'Pending';
...
const getStatus = (s?: string | null): StatusConfig => STATUS[normaliseStatus(s)] ?? STATUS.Pending;
...
const MethodIcon = ({ method }: MethodIconProps) => {
...
// ─── Helpers ──────────────────────────────────────────────────────────────────
const fmt = (n: unknown) => `₦${Number(n).toLocaleString('en-NG')}`;
...
const groupByDate = (payments: any[]): [string, any[]][] => {
...
function TxRow({ payment, onClick, index }: TxRowProps) {
...
interface SummaryStripProps {
  ...
  chartData: { date: string; amountPaid: unknown }[];
}

function SummaryStrip({ totalSpent, counts, chartData }: SummaryStripProps) {
  return (
    <div
...
function FilterTabs({ active, onChange }: FilterTabsProps) {
  ...
  return (
    <div className="flex items-center gap-2 px-4 mb-4 overflow-x-auto pb-0.5">
      {tabs.map((t) => {
        const s = STATUS[t as keyof typeof STATUS];
...
interface DetailDrawerProps {
  payment: any;
  onClose: () => void;
  openCheckout: (payment: any) => void;
}

function DetailDrawer({ payment, onClose, openCheckout }: DetailDrawerProps) {
  const s = getStatus(payment.status);
  console.log('payment', payment);
  const Icon = s.icon;
  const getConfirmationRoute = (payment: any) => {
    const reservationType = payment?.metadata?.reservationType || payment?.vendor?.vendorType;
...
const PaymentsHistory = () => {
  ...
  const [paymentForCheckout, setPaymentForCheckout] = useState<any>(null);
  ...
  const openCheckout = (payment: any) => {
...
const paymentBookingAdapter = (p: any) => ({
