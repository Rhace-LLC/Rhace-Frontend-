  ...
  useEffect(() => {
    const handleCallback = async () => {
      ...
      try {
        ...
        let verifyRes: any, completeRes: any;
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50">
        <div className="text-center p-12 max-w-lg mx-auto bg-white/90 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/50">
          <UniversalLoader />
          <h2 className="mt-6 text-2xl font-bold text-gray-900">Finalizing Your Booking</h2>
          <p className="mt-3 text-gray-600">
            {trxref
              ? `Transaction: ${trxref.slice(-8)}`
              : reference
                ? `Ref: ${reference.slice(-8)}`
                : 'Processing...'}
          </p>
          <p
            className={`mt-4 text-sm font-semibold px-4 py-2 rounded-full bg-gradient-to-r ${
              status === 'success'
                ? 'from-emerald-100 to-green-100 text-emerald-800'
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-rose-50 to-red-50">
        <div className="text-center p-12 max-w-lg mx-auto bg-white/90 backdrop-blur-xl rounded-3xl shadow-2xl border border-red-100">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-4">Reservation Processing Failed</h2>
          <p className="text-gray-600 mb-8 max-w-md mx-auto leading-relaxed">{resultData.error}</p>
