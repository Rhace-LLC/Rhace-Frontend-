...
export const formatOfferText = (offer?: string): string => {
  if (!offer) return '';
...
export const trimLongString = (str?: string, num?: number): string | undefined => {
  if (str && num) {
...
export const routeIs = (path: string): boolean => window.location.pathname === path;
