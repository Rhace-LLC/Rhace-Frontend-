  ...
  useEffect(() => {
    const fetchRestaurant = async () => {
      try {
        ...
        setRestaurant({
          ...res.data,
          selected: res.data.items.map((item: any) => ({ ...item, quantity: 1 })),
        });
      } catch (error) {
        console.error(error);
      } finally {
                ...
                <div className="w-full overflow-clip h-[270px] relative">
                  <div className="absolute top-[20px] px-[16px] z-40 w-full flex md:hidden justify-between items-center">
                    ...
                    <div className="flex gap-2">
                      <button className="size-10 rounded-xl bg-white text-black hover:bg-gray-50 flex justify-center border border-[#E5E7EB] items-center gap-2">
                        <svg
                          width="20"
                          height="20"
                          viewBox="0 0 20 20"
                ...
                <div className="space-y-2">
                  <div className="flex flex-col md:flex-row md:justify-between md:items-cente w-full gap-4">
                    <div className="flex gap-2 items-center pt-2 md:pt-0 px-4 md:px-0">
                      <h1 className="text-2xl text-[#111827] font-semibold">{restaurant.name} </h1>{' '}
                      <span className="px-2 py-0.5 rounded-full border border-black bg-[#D1FAE5]">
                        {' '}
                        {restaurant.vendor.businessName}
                      </span>
                    </div>
                    <RestaurantSaveCopy type="offers" id={id} />
                  </div>
                </div>
              </div>
            </div>
            <div className="col-span-2">{/* <RestaurantInfo data={restaurant} /> */}</div>
          </div>
            ...
            <div className="max-w-sm w-full p-4 rounded-2xl bg-white space-y-4 text-sm text-gray-800">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-2">
                  ...
                  <a href={`mailto:${restaurant.vendor.email}`} className="hover:underline">
